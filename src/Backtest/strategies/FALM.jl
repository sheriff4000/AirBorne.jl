...
function compute_portfolio!(context::ContextTypeA; data=DataFrame())
    ...
    for t in context.extra.tickers
        ...
        # CUSTOM FORECAST ##
        forecast = applyForecast(context.extra.forecastFun, prices; F=context.extra.lookahead)
    ...
    min_returns(x) = sum(best_returns[t] * x[i] for (i,t) in enumerate(context.extra.tickers)) >= context.extra.min_returns_threshold
    ############################
    # constraints #
    ###########################
    ...
    if !max_alloc(init_point) || !long_only(init_point) || !min_alloc(init_point) || !min_returns(init_point)
        init_point = [round(1/length(context.extra.tickers); digits=3) for t in context.extra.tickers]
    end

    weights_problem = DSProblem(dim, objective=obj, granularity=[0.001 for _ in context.extra.tickers], initial_point=init_point)
