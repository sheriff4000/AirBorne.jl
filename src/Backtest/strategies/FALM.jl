...
using AirBorne.Forecast.Linear: LinearForecaster, applyForecast
...
function compute_portfolio!(context::ContextTypeA; data = DataFrame())
	...
	max_alloc(x) = sum(x) <= 1.000 #Bound between something and 1
	long_only(x) = all(x .>= 0)
	...
	if !max_alloc(init_point) ||
	   ...
	   !min_returns(init_point)
		...
		init_point[end] = round(1 - sum(init_point[1:(end-1)]); digits = 3)
	end
